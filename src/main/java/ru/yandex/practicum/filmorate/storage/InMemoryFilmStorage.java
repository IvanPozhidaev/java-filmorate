...
public class InMemoryFilmStorage implements FilmStorage {
    ...
    void validateFields(Film film) {
        if (film.getName().isBlank()) {
            ...
            throw new ValidationException("Пустое поле вместо названия фильма недопустимо");
        }
    ...
    public Film update(Film film) {
        ...
        if (!films.containsKey(film.getId())) {
            throw new ObjectNotFoundException("Обновление данных о фильме невозможно - такого фильма нет в базе");
        }
        films.put(film.getId(), film);
